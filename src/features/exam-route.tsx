...
import { playWrongEffect } from '../lib/audio';
...
export function ExamRoute() {
  ...
  const answer = (correct: boolean) => { if (!correct) { playWrongEffect(); setResult('failed'); return; } if (index === totalQuestions - 1) { markSetCompleted(readCompletedSets(), setId ?? ''); setResult('passed'); return; } setIndex((value) => value + 1); setInput(''); };
