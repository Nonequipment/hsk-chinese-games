...
export function ModeHub({ kind }: { kind: 'games' | 'exams' }) {
  ...
  const removeSet = (id: string) => setSelected((current) => current.length === 1 ? current : current.filter((value) => value !== id));
  ...
  return <main className="mode-hub game-picker"><p className="eyebrow">GAME LIBRARY</p><h1>เลือกคำศัพท์สำหรับเกม</h1><p>เริ่มด้วย Set 01 แล้วเพิ่มเซ็ตที่ต้องการได้จากรายการเดียว</p><section className="selected-sets"><header><div><b>ชุดที่เลือก</b><small>{selected.length} เซ็ต · {selected.length * 20} คำ</small></div><button onClick={() => setSelected(['S01'])}>เริ่มเลือกใหม่</button></header><div className="selected-set-chips">{selectedSets.map((set) => <button key={set.id} className="selected-set-chip" onClick={() => removeSet(set.id)} aria-label={`ลบ ${set.id}`}><b>{set.id}</b><span>{set.category}</span>{selected.length > 1 && <i>×</i>}</button>)}</div><div className="add-set-row"><label>เพิ่มชุดคำศัพท์<select aria-label="เพิ่มเซ็ตในเกม" value={pendingSet} onChange={(event) => setPendingSet(event.target.value)}>{data.sets.map((set) => <option key={set.id} value={set.id} disabled={selected.includes(set.id)}>{set.id} · {set.category}</option>)}</select></label><button aria-label={`เพิ่ม ${pendingSet.replace('S', 'SET ')}`} onClick={addSet} disabled={selected.includes(pendingSet)}>+ เพิ่มเซ็ต</button></div><Link className="play-selected" to={`/practice/${selected.join('+')}`}>เริ่มเล่น {selected.length} เซ็ตที่เลือก <span>→</span></Link></section><section className="quick-game-sets"><h2>เลือกด่วน</h2><p>แตะเพื่อแทนชุดที่เลือกด้วยเซ็ตเดียว</p><div>{data.sets.slice(0, 6).map((set) => <button key={set.id} onClick={() => { setSelected([set.id]); setPendingSet(set.id === 'S01' ? 'S02' : 'S01'); }}><b>{set.id}</b><span>{set.category}</span></button>)}</div></section></main>;
}
