...
export function PracticeRoute() {
  ...
  const reshuffle = () => { setItems((current) => current ? shuffleItems(current) : current); setIndex(0); setScore(0); setInput(''); setFeedback(''); };
  return <main className="activity-page"><Link className="back-link" to="/games">← กลับไปเลือกชุดเกม</Link><p className="eyebrow">PRACTICE LAB</p><h1>เกมฝึกฝน</h1><p className="activity-subtitle">กำลังเล่น {setId?.split('+').join(' + ')} · {items.length} คำ</p><div className="mode-tabs">{modes.map((entry) => <button key={entry.id} className={mode === entry.id ? 'selected' : ''} onClick={() => { setMode(entry.id); reshuffle(); }}>{entry.label}</button>)}</div><section className="game-card"><header><span>ข้อ {index + 1} / {items.length}</span><b>ถูก {score}</b></header><div className="game-settings"><button aria-label="สุ่มข้อใหม่" onClick={reshuffle}>⤨ สุ่มข้อใหม่</button><button aria-label={autoSound ? 'ปิดเสียงอัตโนมัติ' : 'เปิดเสียงอัตโนมัติ'} onClick={toggleAutoSound}>{autoSound ? '🔊 เสียงอัตโนมัติ' : '🔇 ปิดเสียงอัตโนมัติ'}</button><button aria-label={showPinyin ? 'ซ่อนพินอิน' : 'แสดงพินอิน'} onClick={togglePinyin}>{showPinyin ? '◉ ซ่อนพินอิน' : '◌ แสดงพินอิน'}</button></div><p className="game-prompt">{mode === 'hanzi' ? item.thai : item.hanzi}</p>{showPinyin && mode !== 'hanzi' && <p className="game-pinyin">{item.pinyin}</p>}{['meaning', 'reading'].includes(mode) ? <div className="game-choices">{answerChoices.map((choice, choiceIndex) => <button key={`${choice}-${choiceIndex}`} onClick={() => next(choice === item[field])}>{choice}</button>)}</div> : <form onSubmit={(event) => { event.preventDefault(); submit(); }}><label>{mode === 'pinyin' ? 'พิมพ์พินอินพร้อมเลขวรรณยุกต์ เช่น ba4ba' : 'พิมพ์ตัวจีน'}</label><input autoCapitalize="none" value={input} onChange={(event) => setInput(event.target.value)} placeholder={mode === 'pinyin' ? 'เช่น ba4ba' : 'พิมพ์ตัวจีน'} /><button>ตรวจคำตอบ</button></form>}{feedback && <p className="game-feedback">{feedback}</p>}</section></main>;
}
