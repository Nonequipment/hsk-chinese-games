...
import { markItemRemembered, readCompletedSets, readRemembered, getDailySetPlan } from './lib/study-progress';
...
function HomeRoute() {
  ...
  return <main className="dashboard"><section className="mission-card"><div className="brand"><b>学</b><span>HSK MISSION</span><small>เป้าหมาย 17 ต.ค.</small></div><p className="eyebrow">ADAPTIVE MISSION CONTROL</p><h1>เรียนจีนทุกวัน<br /><i>ไปให้ถึง HSK 4.0</i></h1><p>1,200 คำ · 60 เซ็ต · เรียนสั้น กระชับ และจำได้จริง</p><div className="hero-actions"><Link to="/learn/S01">เริ่ม Set แรก →</Link>{username ? <button className="profile-button" aria-label={`โปรไฟล์ ${username}`}>◉ {username}</button> : <button onClick={() => setSignInOpen(true)}>เข้าสู่ระบบ</button>}</div><em className="hero-hanzi">汉</em></section><section className="stats"><div><b>{completed}</b><span>เซ็ตที่สอบผ่าน</span></div><div><b>{completed * 20}</b><span>คำที่สอบผ่าน</span></div><div><b>{plan.remainingSets}</b><span>เซ็ตที่เหลือ</span></div></section><div className="section-title"><span><p className="eyebrow">TODAY’S MISSION</p><h2>แผนของคุณ</h2></span><b>เหลือ {plan.remainingDays} วัน</b></div><section className="today-card"><i>⚡</i><span><strong>วันนี้เรียน {plan.setsPerDay} เซ็ต · {plan.wordsPerDay} คำ</strong><p>เหลือ {plan.remainingSets} เซ็ต เพื่อทันเป้าหมาย 17 ต.ค.</p><small>เรียนให้จบ แล้วสอบผ่าน 100% จึงนับความคืบหน้า</small></span><Link to="/learn/S01" aria-label="เริ่มเรียนตามแผน">→</Link></section><section className="steps"><div><b>01</b><span><strong>เรียนบัตรคำ</strong><small>คำจีน พินอิน คำแปล และเสียง</small></span></div><div><b>02</b><span><strong>ฝึกความจำ</strong><small>ทบทวนจนจำได้อย่างมั่นใจ</small></span></div><div><b>03</b><span><strong>สอบผ่าน 100%</strong><small>เมื่อผ่านแล้วจึงนับเป็น 1 เซ็ตในแผน</small></span></div></section><SignInSheet open={signInOpen} onClose={() => setSignInOpen(false)} /></main>;
}
...
function LibraryRoute() {
  ...
  const done = new Set<string>(readCompletedSets());
