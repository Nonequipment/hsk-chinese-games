...
export function FlashcardSession({set,initialProgress,saveCard=defaultSave}:{set:VocabularySet;initialProgress:string[];saveCard?:SaveCard}){
  ...
  return <div className="mx-auto max-w-2xl"><div className="mb-3 flex items-center justify-between text-sm text-slate-400"><span>{set.id} · ใบที่ {index+1}/20</span><span>{studied.size}/20 เรียนแล้ว</span></div><div className="h-1.5 overflow-hidden rounded-full bg-white/10"><div className="h-full bg-gradient-to-r from-blue-500 to-cyan-300 transition-all" style={{width:`${(index+1)/20*100}%`}}/></div>
    <section className="mt-3 min-h-[300px] rounded-[2rem] border border-white/10 bg-white/5 p-4 text-center shadow-2xl shadow-blue-950/30 sm:mt-4 sm:min-h-[390px] sm:p-8"><p className="text-xs uppercase tracking-[.2em] text-slate-500">{item.category}</p><div className="mt-5 break-words text-6xl font-semibold text-white sm:mt-9 sm:text-7xl">{item.hanzi}</div><button onClick={()=>speakMandarin(item.hanzi)} aria-label="ฟังเสียง" className="mt-4 inline-grid size-12 place-items-center rounded-full bg-blue-500/15 text-cyan-300 sm:mt-5"><Volume2/></button>
      {!revealed?<button onClick={()=>setRevealed(true)} className="mx-auto mt-6 flex min-h-12 items-center gap-2 rounded-2xl bg-blue-500 px-6 font-semibold sm:mt-8"><Eye className="size-5"/> เปิดคำตอบ</button>:<div className="mt-5 sm:mt-6"><p className="text-2xl text-cyan-300">{item.pinyin}</p><p className="mt-2 text-lg text-slate-200">{item.thai}</p><button onClick={()=>setStroke(!stroke)} className="mt-3 inline-flex min-h-11 items-center gap-2 rounded-xl border border-white/15 px-4 text-sm"><PenTool className="size-4"/> ลำดับขีด</button>{stroke&&<div className="mt-5"><StrokeOrder hanzi={item.hanzi}/></div>}<div className="mt-4 sm:mt-6"><MemoryControls onChoose={choose} disabled={saving}/></div></div>}
    </section><div className="mt-3 flex justify-between"><button disabled={index===0} onClick={()=>setIndex(index-1)} className="min-h-11 rounded-xl border border-white/10 px-4 disabled:opacity-30">ก่อนหน้า</button><button disabled={index===19} onClick={()=>setIndex(index+1)} className="min-h-11 rounded-xl border border-white/10 px-4 disabled:opacity-30">ถัดไป</button></div></div>;
}
