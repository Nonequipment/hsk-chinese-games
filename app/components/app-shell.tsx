    ...
    <div className="min-h-dvh overflow-x-hidden bg-[#050817] text-slate-100">
      ...
      <header className="relative border-b border-white/10 bg-[#050817]/80 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 sm:px-6">
          <a href="/" className="flex items-center gap-3 font-semibold tracking-tight"><span className="grid size-10 place-items-center rounded-xl bg-gradient-to-br from-blue-500 to-violet-600 text-xl shadow-lg shadow-blue-500/20">学</span><span>HSK Mission</span></a>
      ...
      <nav aria-label="เมนูหลัก" className="fixed inset-x-0 bottom-0 z-20 border-t border-white/10 bg-[#080d20]/95 px-[max(.5rem,env(safe-area-inset-left))] pb-[max(.45rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur-xl sm:left-1/2 sm:bottom-5 sm:w-[min(34rem,calc(100%-2rem))] sm:-translate-x-1/2 sm:rounded-2xl sm:border">
        <div className="grid grid-cols-4 gap-1">
          {navItems.map(({ id, label, href, Icon }) => <a key={id} href={href} aria-current={active === id ? "page" : undefined} className={`flex min-h-12 flex-col items-center justify-center gap-1 rounded-xl px-2 text-[11px] transition focus-visible:outline-2 focus-visible:outline-cyan-300 ${active === id ? "bg-blue-500/20 text-cyan-200" : "text-slate-400 hover:bg-white/5 hover:text-white"}`}><Icon className="size-5" aria-hidden="true" /><span>{label}</span></a>)}
        </div>
      </nav>
    </div>
  );
